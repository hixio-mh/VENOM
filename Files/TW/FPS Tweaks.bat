...
CLS
ECHO Toggle Dark Mode in Windows
setlocal
set "RegPath=HKCU\Software\Microsoft\Windows\CurrentVersion\Themes"
set "ValueName=AppsUseLightTheme"
set "CurrentValue="

for /f "tokens=2" %%i in ('reg query "%RegPath%" /v "%ValueName%"') do set "CurrentValue=%%i"
...
endlocal
CLS
ECHO Toggle Num Lock state on startup
setlocal

set "NumLockState="
for /f "tokens=2" %%a in ('powershell -command "[console]::CapsLock"') do set "NumLockState=%%a"

if "%NumLockState%"=="False" (
    Reg Add "HKCU\Control Panel\Keyboard" /v "InitialKeyboardIndicators" /t REG_SZ /d "2" /f
    echo Num Lock will be enabled on next startup.
) else (
    exit
)
...
if (-not (Test-Path $regPath)) {
    ...
    Set-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders' -Name 'Documents' -Value '$env:userprofile\Documents' -Type ExpandString
...
"
echo OneDrive removal SCript executed.
CLS
echo.
echo.═══════════════════════════════════════════════════
